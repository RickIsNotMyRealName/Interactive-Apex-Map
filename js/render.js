...
export function drawAll() {
    ...
    state.entityTypeConfig.forEach((c, i) => {
        /* skip zipline entry, group rows, or anything unchecked */
        if (i === zipIdx || c.isGroup || !state.entityTypeFilters[i]) return;
        ...
        state.entData.forEach(item => {
            ...
            item.ents.forEach(e => {
                ...
                if (c.renderType === 'icon' && state.iconCache[i]?.complete) {
                    ...
                    const or = c.outerRadius || 8;
                    const ir = c.innerRadius || 4;

                    ctx.setTransform(1, 0, 0, 1, 0, 0);
                    ctx.fillStyle = c.outerColor || 'lime';
                    ctx.beginPath();
                    ctx.arc(sx, sy, or, 0, 2 * Math.PI);
                    ctx.fill();

                    if (c.innerColor) {
                        ctx.fillStyle = c.innerColor;
                        ctx.beginPath();
                        ctx.arc(sx, sy, ir, 0, 2 * Math.PI);
                        ctx.fill();
                    }

                } else if (c.renderType === 'text') {
                    let txt = c.textField ? (e.props[c.textField] || '') : (c.text || '');
                    if (!txt) return;
                ...
                } else if (c.renderType === 'staticText') {
                    // 1) grab the literal text
                    const txt = c.text || '';
                    if (!txt) return;

                    // 2) pull world‐coords from the config
                    const [worldX, worldY] = c.origin || [0, 0];
                    ...
                    // 4) reset any prior transforms
                    ctx.setTransform(1, 0, 0, 1, 0, 0);

                    // 5) compute font size (optionally scaling with zoom)
                    const base = c.fontSize || 14;
                    const fs = c.scaleWithZoom ? base * state.transform.scale : base;
                    ...
                    // 6) finally, draw it at the computed position + any manual offset
                    ctx.fillText(txt, sx + (c.offsetX || 0), sy + (c.offsetY || 0));
                }
            });
        });
...
function worldToCanvas(x, y) {
    ...
    const cw = getElem('mapCanvas').width;
    const ch = getElem('mapCanvas').height;
    const cx = (x - W.minX) / (W.maxX - W.minX) * cw;
    const cy = ch - ((y - W.minY) / (W.maxY - W.minY) * ch);
    return { cx, cy };
}
